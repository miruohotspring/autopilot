...
COPILOT_FILE="$STATE_DIR/autopilot.copilot"
...
# Case 1:
# Outside tmux: should create session and attach to autopilot:copilot.
...
assert_file_contains "$state1/tmux.log" "new-window -t autopilot -n copilot"
assert_file_contains "$state1/tmux.log" "claude --model sonnet --dangerously-skip-permissions"
assert_file_contains "$state1/tmux.log" "attach-session -t autopilot:copilot"
assert_file_not_contains "$state1/tmux.log" "switch-client -t autopilot:copilot"
...
assert_file_contains "$state2/tmux.log" "new-window -t autopilot -n copilot"
assert_file_contains "$state2/tmux.log" "switch-client -t autopilot:copilot"
assert_file_not_contains "$state2/tmux.log" "attach-session -t autopilot:copilot"

# Case 3:
# Existing session with copilot should reuse and attach.
echo "[test] reuses existing autopilot session with copilot window"
state3="$TMP_DIR/state3"
home3="$TMP_DIR/home3"
mkdir -p "$state3" "$home3/.autopilot"
touch "$state3/autopilot.session"
touch "$state3/autopilot.copilot"
...
assert_file_not_contains "$state3/tmux.log" "new-window -t autopilot -n copilot"
assert_file_contains "$state3/tmux.log" "list-windows -t autopilot -F #{window_name}"
assert_file_contains "$state3/tmux.log" "attach-session -t autopilot:copilot"
assert_file_not_contains "$state3/tmux.log" "switch-client -t autopilot:copilot"

# Case 4:
# Existing session without copilot should add the window and attach.
echo "[test] adds copilot window to existing autopilot session"
...
if [[ "$status4" -ne 0 ]]; then
  echo "assert failed: expected ap briefing to add missing copilot window" >&2
  exit 1
fi
...
assert_file_contains "$state4/tmux.log" "new-window -t autopilot -n copilot"
assert_file_not_contains "$state4/tmux.log" "switch-client -t autopilot:copilot"
assert_file_contains "$state4/tmux.log" "attach-session -t autopilot:copilot"
